...
{
    public class PhotoNameBannedKeywordsError : ErrorInfo
    {
        public PhotoNameBannedKeywordsError(string target, string keyword)
            : base(target)
        {
            Keyword = keyword;
        }
        ...
        public override string Message
        {
            get
            {
                if (IsShowKeywordContent)
                    return "相片名称包含了被禁止的关键字：" + Keyword;
                else
                    return "相片名称包含了被禁止的关键字.";
            } //老达TODO:放到资源项目
        }
