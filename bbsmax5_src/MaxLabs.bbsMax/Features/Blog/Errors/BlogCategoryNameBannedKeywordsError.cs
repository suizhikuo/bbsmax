...
{
	class BlogCategoryNameBannedKeywordsError: ErrorInfo
    {
        public BlogCategoryNameBannedKeywordsError(string target, string keyword)
            : base(target)
        {
            Keyword = keyword;
        }
        ...
        public override string Message
        {
            get
            {
                if (IsShowKeywordContent)
                    return "日志分类名称中包含了被禁止的关键字：" + Keyword;
                else
                    return "日志分类名称中包含了被禁止的关键字";
            }
        }
