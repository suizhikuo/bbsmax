...
{
    public class DoingContentBannedKeywordsError : ErrorInfo
    {
        public DoingContentBannedKeywordsError(string target, string keyword)
            : base(target)
        {
            Keyword = keyword;
        }
        ...
        public override string Message
        {
            get
            {
                if (IsShowKeywordContent)
                    return "记录中包含了被禁止的关键字：" + Keyword;
                else
                    return "记录中包含了被禁止的关键字.";    
            }
        }
