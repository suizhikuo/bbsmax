...
{
	[OperationType(Doing_DeleteDoingBySearch.TYPE_NAME)]
	public class Doing_DeleteDoingBySearch : Operation
	{
        public const string TYPE_NAME = "删除记录";

		public Doing_DeleteDoingBySearch(int operatorID, string operatorName, string operatorIP, Filters.AdminDoingFilter filter, int totalDeleted)
		...
		public Filters.AdminDoingFilter Filter
		{
			get;
			private set;
		}
		...
		public override string Message
		{
			get
			{
				...
				if (Filter.Content != null && Filter.Content != string.Empty)
				{
					sb += "，并且内容中包含关键字“" + Filter.Content + "”";
				}

				if (Filter.IP != null && Filter.IP != string.Empty)
					sb += "，并且作者IP是" + Filter.IP;
				...
				return string.Format(
					"<a href=\"{0}\">{1}</a> 使用删除搜索结果功能，删除了所有 {2} 的记录，共计{3}条数据"
