...
namespace MaxLabs.Passport.InstructEngine
{
    ...
    /// PASSPORT 指令引擎
    /// </summary>
    public class InstructEngine : IInstructEngine
    {
        #region 系统事件接口
        ...
        public InstructEngine()
        {
            this.DriverList = new Dictionary<int, InstructDriver>();
            foreach (PassportClient client in PassportBO.Instance.GetAllPassportClient())
            {
                this.DriverList.Add(client.ClientID, new InstructDriver(client));
            }
