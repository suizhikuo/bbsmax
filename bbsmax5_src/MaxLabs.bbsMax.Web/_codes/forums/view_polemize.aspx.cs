...
namespace MaxLabs.bbsMax.Web.max_pages.forums
{
    ...
    /// Post 的摘要说明
    /// </summary>
    public partial class view_polemize : view_thread
    {
        ...
        {
            if (_Request.IsClick("AgreePolemize"))
            {
                HadProcessSubmit = true;
                ProcessPolemize(ViewPointType.Agree);
                UpdateView = false;
            }
            if (_Request.IsClick("AgainstPolemize"))
            {
                HadProcessSubmit = true;
                UpdateView = false;
        ...
        protected override BasicThread Thread
        {
            get
            {
                return PolemizeThread;
            }
        }

        private PolemizeThreadV5 m_PolemizeThread;
        protected PolemizeThreadV5 PolemizeThread
        {
            get
            {
                if (m_PolemizeThread == null)
                {
                    GetThread();
                }
                return m_PolemizeThread;
            }
        }


        protected override bool MustRemovePostListFirstOne
        {
            get
            {
                if (CurrentPostType != null && (CurrentPostType == PostType.Polemize_Against || CurrentPostType == PostType.Polemize_Agree || CurrentPostType == PostType.Polemize_Neutral))
                    return false;
                return base.MustRemovePostListFirstOne;
            }
        }
        ...
        protected override void GetThread()
        {
            ...
            if (string.IsNullOrEmpty(Type))
            {
                ...
                PostBOV5.Instance.GetPolemizeWithReplies(ThreadID, CurrentPostType, PageNumber, PageSize, true, UpdateView, out m_PolemizeThread, out m_PostList, out realThreadType, out totalCount);
            ...
            {
                BasicThread thread;
                GetPosts(ThreadType.Polemize, out m_TotalPosts, out thread, out m_PostList);

                m_PolemizeThread = (PolemizeThreadV5)thread;
        ...
        protected override bool IsShowPostIndexAlias
        {
            get
            {
                if (CurrentPostType != null)
                    return false;
                return base.IsShowPostIndexAlias;
            }
        }

        private bool hadSetPostTypeValue = false;
        private PostType? m_CurrentPostType;
        protected PostType? CurrentPostType
        {
            get
            {
                if (hadSetPostTypeValue == false)
                {
                    hadSetPostTypeValue = true;
                    m_CurrentPostType = _Request.Get<PostType>("posttype", Method.Get);
                }
                return m_CurrentPostType;
            }
        }

        private bool hasSetMyViewPointType = false;
        private ViewPointType? m_MyViewPointType;
        protected ViewPointType? MyViewPointType
        {
            get
            {
                if (hasSetMyViewPointType == false)
                {
                    hasSetMyViewPointType = true;
                    if (MyUserID != 0)
                    {
                        ViewPointType pointType;
                        if (PolemizeThread.PoleMizeUsers.TryGetValue(MyUserID, out pointType))
                        {
                            m_MyViewPointType = pointType;
                        }
                    }
                }
                return m_MyViewPointType;
            }
        }


        private int? m_PolemizeUserCount;
        protected int PolemizeUserCount
        {
            get
            {
                if (m_PolemizeUserCount == null)
                {
                    m_PolemizeUserCount = PolemizeThread.AgainstCount + PolemizeThread.AgreeCount + PolemizeThread.NeutralCount;
                }
                return m_PolemizeUserCount.Value;
            }
        }

        private double? m_AgreePercent;
        protected double AgreePercent
        {
            get
            {
                if (m_AgreePercent == null)
                {
                    m_AgreePercent = GetPercent(PolemizeThread.AgreeCount, PolemizeUserCount);
                }
                return m_AgreePercent.Value;
            }
        }

        private double? m_AgainstPercent;
        protected double AgainstPercent
        {
            get
            {
                if (m_AgainstPercent == null)
                {
                    m_AgainstPercent = GetPercent(PolemizeThread.AgainstCount, PolemizeUserCount);
                }
                return m_AgainstPercent.Value;
            }
        }

        private double? m_NeutralPercent;
        protected double NeutralPercent
        {
            get
            {
                if (m_NeutralPercent == null)
                {
                    m_NeutralPercent = GetPercent(PolemizeThread.NeutralCount, PolemizeUserCount);
                }
                return m_NeutralPercent.Value;
            }
        }
