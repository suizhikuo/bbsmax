...
namespace Gif.Components
{
	public class GifDecoder 
	{
		...
		{
			public GifFrame( Image im, int del, Color tran, int dis) 
			{
				image = im;
				delay = del;
				tranColor = tran;
			...
			{
				return "delay:" + delay + "tranColor:" + tranColor + "dispose:" + dispose;
			}
		}

		public Color GetTransparent(int n)
		{
			Color color = Color.Empty;

			if ((n >= 0) && (n < frameCount))
			{
				color = ((GifFrame)frames[n]).tranColor;
			}

			return color;
		}

		public int GetDispose(int n)
		{
			//
			int dispose = -1;
			if ((n >= 0) && (n < frameCount))
			{
				dispose = ((GifFrame)frames[n]).dispose;
			}
			return dispose;
		}
		...
		protected void SetPixels() 
		{
			...
			if (lastDispose > 0) 
			{
				if (lastDispose == 3 || lastDispose == 1) 
				{
		...
		protected void ReadImage() 
		{
			...
			frames.Add(new GifFrame(bitmap, delay, transparency ? Color.FromArgb(save) : Color.Empty, dispose)); // add image to frame list
